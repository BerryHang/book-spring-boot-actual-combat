...
import com.learning.spring.boot.controller.common.BaseController;
...
import com.learning.spring.boot.util.FileUtil;
...
public class MenuController extends BaseController {
    ...
    public ResponseBo getMenu(String userName) {
        try {
            List<TMenu> menus = menuService.findUserMenus(userName);
    ...
    public ResponseBo getMenu(Long menuId) {
        try {
            TMenu menu = menuService.findById(menuId);
    ...
    public ResponseBo getMenuButtonTree() {
        try {
            Tree<TMenu> tree = menuService.getMenuButtonTree();
    ...
    public ResponseBo getMenuTree() {
        try {
            Tree<TMenu> tree = menuService.getMenuTree();
            return ResponseBo.ok(tree);
        } catch (Exception e) {
    ...
    public ResponseBo getUserMenu(String userName) {
        try {
            Tree<TMenu> tree = menuService.getUserMenu(userName);
            return ResponseBo.ok(tree);
        } catch (Exception e) {
    ...
    @RequiresPermissions("menu:list")
    @ResponseBody
    public List<TMenu> menuList(Menu menu) {
        try {
    ...
    public ResponseBo menuExcel(Menu menu) {
        try {
            List<TMenu> list = menuService.findAllMenus(menu);
    ...
    public boolean checkMenuName(String menuName, String type, String oldMenuName) {
        ...
        TMenu result = menuService.findByNameAndType(menuName, type);
        return result == null;
    }
    ...
    public List<Map<String, String>> getAllUrl() {
        return menuService.getAllUrl("1");
    }

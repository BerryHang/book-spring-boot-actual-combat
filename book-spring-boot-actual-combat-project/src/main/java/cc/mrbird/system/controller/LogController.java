...
public class LogController extends BaseController {
    ...
    @ResponseBody
    public ResponseBo logExcel(SysLog sysLog) {
        try {
            List<SysLog> list = this.logService.findAllLogs(sysLog);
            return FileUtil.createExcelByPOIKit("系统日志表", list, SysLog.class);
        } catch (Exception e) {
            log.error("导出系统日志Excel失败", e);
    ...
    @ResponseBody
    public ResponseBo logCsv(SysLog sysLog) {
        try {
            List<SysLog> list = this.logService.findAllLogs(sysLog);
            return FileUtil.createCsv("系统日志表", list, SysLog.class);
        } catch (Exception e) {
            log.error("导出系统日志Csv失败", e);
